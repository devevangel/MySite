...
I hit this building paid challenges on a browser game. Admins wanted new rules weekly. Engineering wanted sleep.

## The wrong first step

Hard-coded templates in the client worked for three challenges. The fourth request was always a combination nobody predicted  - timer AND kill count AND item restriction. Multiplying hand-written templates does not scale.
...
Client UI can preview progress (timer, counters) for responsiveness. Who may credit a win stays on the server, using the same frozen config the anti-cheat replay trusts.
