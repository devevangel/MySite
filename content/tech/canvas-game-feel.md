...
status: published
summary: "Custom canvas engine tricks: reused particles, directional hit feedback, and distance-based sound."
...
Allocating objects every explosion is an easy way to stutter. I kept a **fixed set** of 400 particles and reused dead ones. On weaker devices, spawn counts scale down so combat frenzies do not flood the GPU.
...
When the player takes damage, a small **wedge** on screen points toward the source. Short fade, a few reused wedges. Players feel where pressure is coming from without reading UI.
...
Reuse, falloff, layered cheap effects, and one quality toggle get you surprisingly far without Unity. The skill is knowing what to skip on weak hardware, not adding more features.
