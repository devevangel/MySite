...
On day one there are no real players. If publish has to wait for a crowd, the board stays empty. If publish guesses, either the reward budget is wrong or the challenge is no fun.
...
Publish still has to pass the rest of the lint: the recipe comes from the catalogue, the item mix is legal, the predicted finish rate sits in the tier band. The only thing bots unlock is "we have a baseline at all."

Until real players confirm the guess, rewards stay smaller than the full table. That is not a business trick. It is the cost of being early. When the human sample is large enough, the cap lifts on its own. Nobody has to remember to flip it.
...
Human runs and bot runs answer different questions. Bots answer "can anyone finish this, and roughly how hard is it?" People answer "how hard is it for this audience?" Using one as the other is how you ship a challenge that only a script can win, or one that nobody will play twice.
